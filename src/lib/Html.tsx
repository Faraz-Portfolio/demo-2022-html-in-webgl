...
import { applyProps, ReactThreeFiber, useFrame, useThree } from '@react-three/fiber'
import { DepthMask } from './DepthMask'
...
export const Html = React.forwardRef(
  (
    ...
    const depthMask = React.useMemo(() => {
      if (depthTest) {
        const state = {
          position: new Vector3()
        }
        applyProps(state as any, { position: props.position })
        return new DepthMask(size.height, size.width, state.position)
      }
      return
    }, [depthTest, size])
    ...
    React.useEffect(() => {
      if (depthMask) {
        ...
        applyProps(state as any, { position: props.position })
        depthMask.setPoint(state.position)
      }
    }, [depthMask, props.position])

    React.useEffect(() => () => depthMask?.destroy(), [depthMask])
    ...
    const transformInnerStyles: React.CSSProperties = React.useMemo(() => ({ position: 'absolute', pointerEvents }), [pointerEvents])
    ...
    useFrame(({ gl, scene, camera }) => {
      depthMask?.update(gl, scene, camera)
      ...
      if (group.current) {
        ...
        if (
          ...
          if (depthMask) {
            const url = depthMask.getMaskAsImage(gl)
            el.style.webkitMaskImage = `url(${url})`
            el.style.webkitMaskClip = `content-box`
